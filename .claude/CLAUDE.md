...
- **Auth Salesforce** : OAuth2 (client_credentials / password grant)
...
└── Service/
    ├── Salesforce/
    │   ├── SalesforceAuthService.php        # OAuth2 token management
...
- `config/services.yaml` — Binding du client HTTP Salesforce
...
|----------|-------------|
| `make init` | **Initialisation complète du projet** (build + start + install + permissions) — À utiliser lors du premier démarrage |
| `make build` | Construit/reconstruit les images Docker |
| `make start` | Démarre les conteneurs Docker en arrière-plan |
| `make stop` | Arrête tous les conteneurs |
| `make restart` | Redémarre tous les conteneurs (stop + start) |
| `make logs` | Affiche les logs de tous les conteneurs en temps réel (Ctrl+C pour quitter) |
| `make shell` | Ouvre un shell bash dans le conteneur PHP (pour exécuter des commandes directement) |

### Gestion des dépendances (Composer)

| Commande | Description |
|----------|-------------|
| `make install` | Installe les dépendances Composer définies dans composer.lock |
| `make update` | Met à jour les dépendances Composer |
| `make require package=vendor/package` | Installe un nouveau package Composer (ex: `make require package=symfony/validator`) |

### Symfony

| Commande | Description |
|----------|-------------|
| `make clear-cache` | Vide le cache Symfony (à faire après modification de config) |
...
make shell                    # Travailler dans le conteneur si besoin
...
make require package=symfony/mailer
